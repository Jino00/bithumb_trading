...
### 대원칙: 이익의 최대화
> **모든 행동과 모든 시도와 모든 학습은 거래를 통한 이익의 최대화를 위한다.**
>
> 이것이 이 시스템의 존재 이유이자 모든 판단의 기준이다.
> 코드를 수정할 때, 전략을 바꿀 때, 파라미터를 조정할 때, 새 기능을 추가할 때
> — 항상 "이것이 이익을 늘리는가?"를 먼저 묻는다.
> 이익에 기여하지 않는 작업은 하지 않는다.

### 학습→인사이트→행동 피드백 루프
- **모든 거래에서 학습한다** — 매 거래 종료 시 TradeInsightEngine이 자동 진단 (왜 이겼는지, 왜 졌는지)
- **학습은 하나도 빠짐없이 인사이트로 남긴다** — trade_insights.json + learning_state.json에 영속 저장
- **인사이트는 반드시 다음 거래에 반영된다** — 기록만 하고 행동하지 않는 인사이트는 없어야 한다
- **반영 경로 6가지** (모두 자동 작동):
  1. 전략×레짐 차단: 승률 20% 미만 조합은 진입 차단 (`InsightActions.blocked_strategy_regimes`)
  2. 티어별 포지션 축소: 승률 25% 미만 변동성 티어는 투자금 축소 (`InsightActions.tier_position_scale`)
  3. SL 자동 확대: SL/SL_FAST 청산 비율 50% 초과 시 SL 넓힘 (`InsightActions.should_widen_sl`)
  4. 워터폴 우선순위 재정렬: 승률 높은 전략이 먼저 시도됨 (`InsightActions.strategy_scores`)
  5. 시간대 차단: 승률 25% 미만 시간대는 진입 차단 (`InsightActions.blocked_hours`)
  6. 연속 손실 쿨다운: N연패 시 거래 일시 중단 (`InsightActions.consecutive_losses`)
- **새 학습 유형을 추가하면** → 반드시 `get_actions()`에 구조화된 필드를 추가하고, 거래 로직에서 소비하는 코드를 연결한다
- **"기록했다" ≠ "반영했다"** — 텍스트 출력만으로는 학습이 아님. 코드로 연결되어야 진짜 학습
